...
class CustomFieldsHandler:
    ...
    @staticmethod
    async def get_values_by_customer(customer_id: str, shop_id: str, session: AsyncSession):
        service = CustomFieldsService(session)
        return await service.get_values_by_customer(customer_id=customer_id, shop_id=shop_id)
