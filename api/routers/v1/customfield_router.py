...
router = APIRouter(prefix="/customers/custom-fields", tags=["Custom Fields"])
...
async def get_values_by_customer(session:PG_ASYNC_SESSION,data:GetvaluesByCustomerId=Depends()):
    return await CustomFieldsHandler.get_values_by_customer(data=data,session=session)
