...
from ..msgqueue_services.customer_services import MessagingQueueCustomerService


MESSAGING_QUEUE_SERVICE_MAPPER_BY_SERVICE_NAME={
    "CUSTOMERS": MessagingQueueCustomerService
}
...
async def service_main_controller(msg:AbstractIncomingMessage):
    async with AsyncInfraDbLocalSession() as session:
        ...
        is_valid_saga = bool(saga_id and str(saga_id).strip().lower() not in ("none", "", "null"))
        try:
            ...
            if not entity_name or not body or not service_name:
                ...
                if is_valid_saga:
                    try:
                        ...
                        ic(f"Failed to record saga cancel: {s_err}")
            ...
            if response is not None:
                ...
                if is_valid_saga:
                    try:
                        ...
                        ic(f"Failed to merge saga response: {s_err}")
                ...
                if is_valid_saga:
                    try:
                        ...
                        ic(f"Failed to record saga error: {s_err}")
            ...
            if is_valid_saga:
                try:
                    ...
                    ic(f"Failed to record fatal saga error: {s_err}")
