...
class CustomFieldsRepo:
    ...
    async def create_field(self, data: CreateCustomFieldDbSchema) -> bool:
        self.session.add(CustomerCustomFields(**data.model_dump(mode='json')))
        return True
    ...
    async def update_field(self, field_id: str, shop_id: str, update_data: dict) -> Optional[str]:
        stmt = (
            ...
            .where(CustomerCustomFields.id == field_id, CustomerCustomFields.shop_id == shop_id)
            .values(**update_data)
    ...
    async def delete_field(self, field_id: str, shop_id: str) -> bool:
        stmt = delete(CustomerCustomFields).where(
            CustomerCustomFields.id == field_id,
    ...
    async def get_field_by_name(self, field_name: str, shop_id: str) -> Optional[dict]:
        stmt = select(CustomerCustomFields).where(
            CustomerCustomFields.field_name == field_name, 
            CustomerCustomFields.shop_id == shop_id
        )
        res = (await self.session.execute(stmt)).scalars().first()
        if res:
    ...
    async def get_all_fields(self, shop_id: str) -> List[dict]:
        stmt = select(CustomerCustomFields).where(CustomerCustomFields.shop_id == shop_id)
    ...
    async def upsert_field_value(self, data: CreateCustomFieldValueDbSchema) -> bool:
        stmt = select(CustomerCustomFieldsValues).where(
            CustomerCustomFieldsValues.customer_id == data.customer_id,
            CustomerCustomFieldsValues.field_id == data.field_id,
        ...
        if existing:
            update_stmt = (
                update(CustomerCustomFieldsValues)
                .where(CustomerCustomFieldsValues.id == existing.id)
                .values(value=data.value)
            )
            await self.session.execute(update_stmt)
        else:
            self.session.add(CustomerCustomFieldsValues(**data.model_dump(mode='json')))
            
        return True
        
    async def get_values_by_customer_id(self, customer_id: str, shop_id: str) -> List[dict]:
        stmt = select(CustomerCustomFieldsValues).where(
            CustomerCustomFieldsValues.customer_id == customer_id,
